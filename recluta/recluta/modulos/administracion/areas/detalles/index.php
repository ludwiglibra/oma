<?PHP
	...
	if ($area_seleccionada != "") {
		...
		$listado_asignados = $core["areas"]->Asignados();
		$listado_disponibles = $core["areas"]->Disponibles();
    	...
    	<?PHP if ($sesion_registrada) { ?>
            ...
            <div class="izquierda">                
                <div class="contenido">
                  ...
                  <h1>Detalles de Area <strong><?PHP echo strtoupper($area_seleccionada); ?></strong></h1>
                  <form name="frmDetalles" method="post" action="actualizar.php">
                    	<table class="detalles">
                        	<tbody>
                            	<tr>
                                	<td class="obligatorio">* ID Área</td>
                                    <td class="campo"><input type="text" name="area" value="<?PHP echo $core["areas"]->getArea(); ?>" size="50" readonly="readonly" class="llave" /></td>
                                </tr>
                            	<tr>
                                	<td class="obligatorio">* Descripción</td>
                                    <td class="campo"><input type="text" name="descripcion" value="<?PHP echo $core["areas"]->getDescripcion(); ?>" size="70" /></td>
                                </tr>
                                <tr>
                                	<td class="obligatorio">Área Activa</td>
                                    <td class="campo">
                                    	<?PHP if ($core["areas"]->getActivo() == "X") { ?>
                    ...
                    <tbody>
                        <?PHP foreach ($listado_asignados as $indice => $elemento) { ?>
                            <tr>
                                <td><?PHP echo $indice + 1; ?></td>
                                <td><?PHP echo $elemento["usuario"]; ?></td>
                                <td><?PHP echo $elemento["nombre"]; ?></td>
                                <td><?PHP echo $elemento["correo"]; ?></td>
                                <td><?PHP echo $elemento["nivel"]; ?></td>
                                <td><a href="desasignar.php?usuario=<?PHP echo $elemento["usuario"]; ?>"><img src="../../../../imgs/utilerias/basurero.png" alt="Desasignar" title="Desasignar" /></a></td>
                            </tr>
                        <?PHP } ?>
                    </tbody>
                    <tfoot>
                        <tr>
                            <td colspan="6">
                            	<form name="frmUsuarios" method="post" action="asignar.php">
                                	<input type="hidden" name="area" value="<?PHP echo $core["areas"]->getArea(); ?>" />
                                	<select name="usuario">
                                    	<option value="">Elija un usuario</option>
                                        <?PHP foreach ($listado_disponibles as $indice => $elemento) { ?>
                                            <option value="<?PHP echo $elemento["usuario"]; ?>"><?PHP echo strtoupper($elemento["usuario"]) . " - " . $elemento["nombre"]; ?></option>
                                        <?PHP } ?>
                                    </select>
