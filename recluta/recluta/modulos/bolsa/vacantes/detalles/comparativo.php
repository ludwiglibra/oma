<?PHP
	...
	$numero_candidatos = count($listado_candidatos);
	
	/* Cargar Datos de Candidatos */
	
	foreach ($listado_candidatos as $indice => $elemento) {
		$variable = "candidato" . $indice;
		$$variable = $core["candidatos"]->DatosComparativo($elemento["candidato"]);
	}
	
	/* Armar Tabla */
	
	$listado_comparativo = array();
	
	/* Edad */
	$elemento = array();
	$elemento["seccion"] = "Edad";
	$elemento["calificacion"] = "";
	foreach ($listado_candidatos as $indice => $elemento2) {
		$variable = "candidato" . $indice;
		$auxiliar = $$variable;
		$elemento[$auxiliar["nombre"]] = $auxiliar["edad"];
	}
	$listado_comparativo[] = $elemento;
	
	/* Estado Civil */
	$elemento = array();
	$elemento["seccion"] = "Estado Civil";
	$elemento["calificacion"] = "";
	foreach ($listado_candidatos as $indice => $elemento2) {
		$variable = "candidato" . $indice;
		$auxiliar = $$variable;
		$elemento[$auxiliar["nombre"]] = $auxiliar["civil"];
	}
	$listado_comparativo[] = $elemento;
	
	/* Dependientes Económicos */
	$elemento = array();
	$elemento["seccion"] = "Dependientes Económicos";
	$elemento["calificacion"] = "";
	foreach ($listado_candidatos as $indice => $elemento2) {
		$variable = "candidato" . $indice;
		$auxiliar = $$variable;
		$elemento[$auxiliar["nombre"]] = $auxiliar["dependientes"];
	}
	$listado_comparativo[] = $elemento;
	
	/* Domicilio */
	$elemento = array();
	$elemento["seccion"] = "Domicilio de la vacante: " . $core["vacantes"]->getUbicacion();
	$elemento["calificacion"] = "";
	foreach ($listado_candidatos as $indice => $elemento2) {
		$variable = "candidato" . $indice;
		$auxiliar = $$variable;
		$elemento[$auxiliar["nombre"]] = $auxiliar["domicilio"];
	}
	$listado_comparativo[] = $elemento;
	
	/* Trayectoria del Candidato */
	$elemento = array();
	$elemento["seccion"] = "Formación esperada: " . $core["vacantes"]->getFormacion();
	$elemento["calificacion"] = "";
	foreach ($listado_candidatos as $indice => $elemento2) {
		$variable = "candidato" . $indice;
		$auxiliar = $$variable;
		$elemento[$auxiliar["nombre"]] = $auxiliar["puesto"];
	}
	$listado_comparativo[] = $elemento;
	
	/* Ingreso a Puesto Laboral */
	$elemento = array();
	$elemento["seccion"] = "Ingreso a Puesto Laboral";
	$elemento["calificacion"] = "";
	foreach ($listado_candidatos as $indice => $elemento2) {
		$variable = "candidato" . $indice;
		$auxiliar = $$variable;
		$elemento[$auxiliar["nombre"]] = $auxiliar["ingreso"];
	}
	$listado_comparativo[] = $elemento;
	
	/* Salario Anterior */
	$elemento = array();
	$elemento["seccion"] = "Salario Anterior";
	$elemento["calificacion"] = "";
	foreach ($listado_candidatos as $indice => $elemento2) {
		$variable = "candidato" . $indice;
		$auxiliar = $$variable;
		$elemento[$auxiliar["nombre"]] = $auxiliar["anterior"];
	}
	$listado_comparativo[] = $elemento;
	
	/* Salario Actual */
	$elemento = array();
	$elemento["seccion"] = "Salario Actual";
	$elemento["calificacion"] = "";
	foreach ($listado_candidatos as $indice => $elemento2) {
		$variable = "candidato" . $indice;
		$auxiliar = $$variable;
		$elemento[$auxiliar["nombre"]] = $auxiliar["actual"];
	}
	$listado_comparativo[] = $elemento;
	
	/* Expectativa Salarial */
	$elemento = array();
	$elemento["seccion"] = "Expectativa Salarial";
	$elemento["calificacion"] = "";
	foreach ($listado_candidatos as $indice => $elemento2) {
		$variable = "candidato" . $indice;
		$auxiliar = $$variable;
		$elemento[$auxiliar["nombre"]] = $auxiliar["expectativa"];
	}
	$listado_comparativo[] = $elemento;
	
	/* Estudios */
	$elemento = array();
	$elemento["seccion"] = "Estudios Requeridos: " . $core["vacantes"]->getConocimientos();
	$elemento["calificacion"] = "";
	foreach ($listado_candidatos as $indice => $elemento2) {
		$variable = "candidato" . $indice;
		$auxiliar = $$variable;
		$elemento[$auxiliar["nombre"]] = $auxiliar["estudios"];
	}
	$listado_comparativo[] = $elemento;
	
	/* Responsabilidades */
	$elemento = array();
	$elemento["seccion"] = "Responsabilidades";
	$elemento["calificacion"] = "";
	foreach ($listado_candidatos as $indice => $elemento2) {
		$variable = "candidato" . $indice;
		$auxiliar = $$variable;
		$elemento[$auxiliar["nombre"]] = $auxiliar["responsabilidades"];
	}
	$listado_comparativo[] = $elemento;
	
	$core["sesion"]->setVariable("sesion_reporte","Comparativo de Candidatos");
	$core["sesion"]->setVariable("sesion_datos", $listado_comparativo);
    	...
    	<?PHP if ($sesion_registrada) { ?>
            ...
            <div class="izquierda">                
                <div class="contenido">
                  ...
                  <h1>Cuadro Comparativo de Vacante <?PHP echo $core["vacantes"]->getTitulo(); ?></h1>
                  
				<table class="reportes">
                    <thead>
                        <tr>
                            <td>Rubros a Evaluar</td>
                            <td>&nbsp;</td>
                            <?PHP
								foreach ($listado_candidatos as $indice => $candidato) {
									echo "<td>Candidato <u><strong>" . $candidato["nombre"] . "</strong></u></td>";
								}
							?>
                        </tr>
                    </thead>
                    <tbody>
                    	<?PHP foreach ($listado_comparativo as $indice1 => $elemento1) { ?>
                        <tr>
                            <?PHP
								foreach ($elemento1 as $indice => $candidato) {
