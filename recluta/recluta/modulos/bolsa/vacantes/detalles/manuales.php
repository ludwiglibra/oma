<?PHP
	...
	$candidato = $core["sesion"]->getVariable("candidato_seleccionado");
	$vacante = $core["sesion"]->getVariable("vacante_seleccionada");
	
	if (!($core["vacantes"]->ExistenManuales($candidato, $vacante))) {
		$core["sesion"]->setMensaje("Se han calificado todas las respuestas abiertas.");
		header("Location: " . $pagina);
		exit;
	}
	
	$listado_manuales = $core["vacantes"]->ListadoManuales($vacante, $candidato);
	...
	<div class="homeCont">
    	...
    	<?PHP if ($sesion_registrada) { ?>
            ...
            <div class="izquierda">                
                ...
                <h1>Calificación Manual Candidato <?PHP echo $candidato; ?> en Vacante <?PHP echo $vacante; ?></h1>
                    <table class="reportes">
                        ...
                        <tbody>
                            <?PHP foreach ($listado_manuales as $indice_manual => $manual) { ?>
                            <form name="frmCalificar" method="post" action="calificar.php">
                            	<input type="hidden" name="candidato" value="<?PHP echo $manual["candidato"]; ?>" />
                                <input type="hidden" name="vacante" value="<?PHP echo $manual["vacante"]; ?>" />
                                <input type="hidden" name="bateria" value="<?PHP echo $manual["bateria"]; ?>" />
                                <input type="hidden" name="evaluacion" value="<?PHP echo $manual["evaluacion"]; ?>" />
                                <input type="hidden" name="pregunta" value="<?PHP echo $manual["pregunta"]; ?>" />
                                <tr>
                                    <td><?PHP echo $manual["bateria"]; ?></td>
                                    <td><?PHP echo $manual["evaluacion"]; ?></td>
                                    <td><?PHP echo $manual["pregunta_txt"]; ?></td>
                                    <td><?PHP echo $manual["respuesta_txt"]; ?></td>
                                    <td>
                                        <select name="valor">
                                            <?PHP for ($x=0; $x<=10; $x++) { ?>
                                                <option value="<?PHP echo ($x * 10); ?>"><?PHP echo ($x * 10); ?></option>
                                            <?PHP } ?>
                                        </select>
                                    </td>
                                    <td>
